...
function convertTimeToNumber(t) {
    const time = t.split(":");
    return ((parseInt(time[0]) * 3600 + parseInt(time[1]) * 60 + parseFloat(time[2])) * 1000);
}
...
function searchPopup() {
    ...
    const timeInputFrom = document.createElement("input");
    timeInputFrom.type = "time";
    timeInputFrom.step = "1";
    timeInputFrom.value = searchCriteria.timeFrom ?? "00:00:00";
    timeInputFrom.id = "search-time-input-from";
    timeInputP.append(timeInputFrom);
    timeInputP.append(" ~ ");
    const timeInputTo = document.createElement("input");
    timeInputTo.type = "time";
    timeInputTo.step = "1";
    timeInputTo.value = searchCriteria.timeTo ?? "00:00:00";
    timeInputTo.id = "search-time-input-to";
    timeInputP.append(timeInputTo);
...
function search() {
    ...
    Object.keys(data.recordList).forEach((r) => {
        Object.keys(data.recordList[r]).forEach((t) => {
            ...
            const timeInputFromEle = document.getElementById("search-time-input-from");
            const timeInputToEle = document.getElementById("search-time-input-to");
            let timeFrom = convertTimeToNumber(timeInputFromEle.value);
            let timeTo = convertTimeToNumber(timeInputToEle.value);
...
function searchCriteriaSave() {
    ...
    const timeFromEle = document.getElementById("search-time-input-from");
    const timeToEle = document.getElementById("search-time-input-to");
    if(timeFromEle.value) {
        searchCriteria.timeFrom = timeFromEle.value;
    }
    if(timeToEle.value) {
        searchCriteria.timeTo = timeToEle.value;
    }
