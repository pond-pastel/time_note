const exampleTags = ["勉強", "運動", "掃除", "料理", "読書", "睡眠", "ゲーム"];
let data;
let searchData, sortData;
let searching = false;
let searchCriteria = {};
let searchTag = [];
            ...
            if(showLapEle.checked) {
                ...
                if(r.lapList.length === 0) {
                    ...
                    r.lapList.forEach((l, i) => {
                        ...
                        const lapTime = `${returnTime(l)}${i === 0 ? "" : `(+${returnTime(l - r.lapList[i - 1])})`}`;
                        title += `ラップ${i + 1}: ${lapTime}`;
...
function showDetails(k, t) {
    ...
    function timerDataApply(timerData, key) {
        ...
        if(timerData.lapList.length > 0) {
            ...
            lap.append(lapTitle);
            const ol = document.createElement("ol");
            timerData.lapList.forEach((l, i) => {
                const li = document.createElement("li");
                li.textContent = `${returnTime(l)}${i === 0 ? "" : `(+${returnTime(l - timerData.lapList[i - 1])})`}`;
                const deleteButton = document.createElement("button");
                deleteButton.textContent = "削除";
                deleteButton.addEventListener("click", () => {
                    toast(`${i + 1}番目のラップを削除しますか？`, recordLapRemove, [(t === "date" ? timerData : k), k, t, i]);
                });
...
function timerCancel(k) {
    ...
    toast(`本当に${data.timerList[k].name === "" ? "ストップウォッチ" : data.timerList[k].name ?? "ストップウォッチ"}を削除しますか？`, removeTimer, [k]);
}
...
function recordLapRemove(timerKey, key, type, lapIndex) {
    console.log(timerKey, type, lapIndex);
    let target;
    Object.keys(data.recordList).forEach((n) => {
        const record = Object.keys(data.recordList[n]).filter((key) => key === timerKey);
        if(record.length > 0) {
            target = n;
        }
    });
    ...
    if(type === "timer") {
        if(target) {
            let newLapList = [];
            for(let i = 0;i < data.recordList[target][timerKey].lapList.length;i++) {
                if(i !== lapIndex) {
                    newLapList.push(data.recordList[target][timerKey].lapList[i]);
                }
            }
            data.recordList[target][timerKey].lapList = newLapList;
        } else {
            console.error("Record not found");
        }
    } else {
        let newLapList = [];
        for(let i = 0;i < timerKey.lapList.length;i++) {
            if(i !== lapIndex) {
                newLapList.push(timerKey.lapList[i]);
            }
        }
        timerKey.lapList = newLapList;
    }
    tableApply();
...
function lapApply() {
    ...
    Object.keys(timer).forEach((k) => {
        ...
        v.lapList.forEach((l, i) => {
            const lap = document.createElement("div");
            lap.textContent = `Lap${i + 1}: ${returnTime(l)}${i === 0 ? "" : `(+${returnTime(l - v.lapList[i - 1])})`}`;
...
function searchPopup() {
    ...
    questionButton.classList.add("help-button");
    ...
    timeInputFromH.value = searchCriteria.timeFromH ? searchCriteria.timeFromH : "";
    ...
    timeInputFromM.value = searchCriteria.timeFromM ? searchCriteria.timeFromM : "";
    ...
    timeInputFromS.value = searchCriteria.timeFromS ? searchCriteria.timeFromS : "";
    ...
    timeInputToH.value = searchCriteria.timeToH ? searchCriteria.timeToH : "";
    ...
    timeInputToM.value = searchCriteria.timeToM ? searchCriteria.timeToM : "";
    ...
    timeInputToS.value = searchCriteria.timeToS ? searchCriteria.timeToS : "";
...
function search() {
    ...
    Object.keys(data.recordList).forEach((r) => {
        Object.keys(data.recordList[r]).forEach((t) => {
            ...
            if(timeFrom > timeTo && (timeFromHEle.value || timeFromMEle.value || timeFromSEle.value) && (timeToHEle.value || timeToMEle.value || timeToSEle.value)) {
                const hBox = searchCriteria.timeFromH ?? 0;
                searchCriteria.timeFromH = searchCriteria.timeToH ?? 0;
                searchCriteria.timeToH = hBox;
                timeFromHEle.value = (searchCriteria.timeFromH ? searchCriteria.timeFromH : "");
                timeToHEle.value = (searchCriteria.timeToH ? searchCriteria.timeToH : "");
                const mBox = searchCriteria.timeFromM ?? 0;
                searchCriteria.timeFromM = searchCriteria.timeToM ?? 0;
                searchCriteria.timeToM = mBox;
                timeFromMEle.value = (searchCriteria.timeFromM ? searchCriteria.timeFromM : "");
                timeToMEle.value = (searchCriteria.timeToM ? searchCriteria.timeToM : "");
                const sBox = searchCriteria.timeFromS ?? 0;
                searchCriteria.timeFromS = searchCriteria.timeToS ?? 0;
                searchCriteria.timeToS = sBox;
                timeFromSEle.value = (searchCriteria.timeFromS ? searchCriteria.timeFromS : "");
                timeToSEle.value = (searchCriteria.timeToS ? searchCriteria.timeToS : "");
            }
            timeFrom = convertTimeToNumber([searchCriteria.timeFromH ?? "", searchCriteria.timeFromM ?? "", searchCriteria.timeFromS ?? ""]);
            timeTo = convertTimeToNumber([searchCriteria.timeToH ?? "", searchCriteria.timeToM ?? "", searchCriteria.timeToS ?? ""]);
...
function searchCriteriaApply() {
    ...
    if(c.timeFromH || c.timeFromM || c.timeFromS || c.timeToH || c.timeToM || c.timeToS) {
        ...
        if(c.timeFromH || c.timeFromM || c.timeFromS) {
            time.append(`${adjustTheDigits(c.timeFromH ?? "", 2)}:${adjustTheDigits(c.timeFromM ?? "", 2)}:${adjustTheDigits(c.timeFromS ?? "", 2)}`);
        }
        time.append(" ~ ");
        if(c.timeToH || c.timeToM || c.timeToS) {
            time.append(`${adjustTheDigits(c.timeToH ?? "", 2)}:${adjustTheDigits(c.timeToM ?? "", 2)}:${adjustTheDigits(c.timeToS ?? "", 2)}`);
        }
    ...
    if(c.lapFrom || c.lapTo) {
        ...
        if(c.lapFrom) {
            lap.append(`${c.lapFrom}個`);
        }
        lap.append(" ~ ");
        if(c.lapTo) {
            lap.append(`${c.lapTo}個`);
        }
...
function searchCriteriaSave() {
    ...
    if(Date.parse(dateFromEle.value) > Date.parse(dateToEle.value) && dateFromEle.value && dateToEle.value) {
        const box = dateFromEle.value;
        dateFromEle.value = dateToEle.value;
        dateToEle.value = box;
    }
    ...
    if(timeFromHEle.value) {
        if(Number(timeFromHEle.value) < 0) {
            timeFromHEle.value = 0;
        }
        searchCriteria.timeFromH = timeFromHEle.value;
    }
    if(timeFromMEle.value) {
        if(Number(timeFromMEle.value) < 0 || Number(timeFromMEle.value) >= 60) {
            timeFromMEle.value = Math.min(Math.max(0, Number(timeFromMEle.value)), 59);
        }
        searchCriteria.timeFromM = timeFromMEle.value;
    }
    if(timeFromSEle.value) {
        if(Number(timeFromSEle.value) < 0 || Number(timeFromSEle.value) >= 60) {
            timeFromSEle.value = Math.min(Math.max(0, Number(timeFromSEle.value)), 59);
        }
        searchCriteria.timeFromS = timeFromSEle.value;
    }
    if(timeToHEle.value) {
        if(Number(timeToHEle.value) < 0) {
            timeToHEle.value = 0;
        }
        searchCriteria.timeToH = timeToHEle.value;
    }
    if(timeToMEle.value) {
        if(Number(timeToMEle.value) < 0 || Number(timeToMEle.value) >= 60) {
            timeToMEle.value = Math.min(Math.max(0, Number(timeToMEle.value)), 59);
        }
        searchCriteria.timeToM = timeToMEle.value;
    }
    if(timeToSEle.value) {
        if(Number(timeToSEle.value) < 0 || Number(timeToSEle.value) >= 60) {
            timeToSEle.value = Math.min(Math.max(0, Number(timeToSEle.value)), 59);
        }
    ...
    if(Number(lapFromEle.value) > Number(lapToEle.value) && lapFromEle.value && lapToEle.value) {
        const box = lapFromEle.value;
        lapFromEle.value = lapToEle.value;
        lapToEle.value = box;
    }
    if(lapFromEle.value) {
...
/*function sortPopup() {
    ...
    const [blur, menu] = showMenu();
    const title = document.createElement("h3");
    title.textContent = "ソート";
    const helpButton = document.createElement("button");
    helpButton.textContent = "？";
    helpButton.classList.add("help-button");
    helpButton.addEventListener("click", () => {
        toast("時間を基準に日付内でソートします。");
    });
    title.append(helpButton);
    menu.append(title);
    const orderBox = document.createElement("p");
    orderBox.append("順序選択: ");
    const order = document.createElement("select");
    order.id = "sort-order";
    const ascendingOrderOpt = document.createElement("option");
    ascendingOrderOpt.textContent = "昇順";
    ascendingOrderOpt.value = "ascending";
    ascendingOrderOpt.selected = true;
    order.append(ascendingOrderOpt);
    const descendingOrderOpt = document.createElement("option");
    descendingOrderOpt.textContent = "降順";
    descendingOrderOpt.value = "descending";
    order.append(descendingOrderOpt);
    ...
    buttonGroup.append(sortButton);
    if(sortData) {
        const sortClearButton = document.createElement("button");
        sortClearButton.textContent = "×ソート解除";
        sortClearButton.addEventListener("click", () => {
            sortClear();
        });
        buttonGroup.append(sortClearButton);
    }
    const cancelButton = document.createElement("button");
    cancelButton.textContent = "×キャンセル";
    cancelButton.addEventListener("click", () => {
...
function sort() {
    const orderEle = document.getElementById("sort-order");
    const order = orderEle.value;
    const record = searchData ?? data.recordList;
    sortData = new Map();
    Object.keys(record).forEach((k) => {
        sortData.set(k, new Map());
        const item = record[k].sort((a, b) => {
            return a.time - b.time;
        });
        Object.keys(record[k]).forEach((r, i) => {
            sortData.get(k).set(r, item[i]);
        });
        console.log(item);
    });
...
function average() {
    ...
    let count = 0;
    Object.keys(record).forEach((k) => {
        Object.keys(record[k]).forEach((r) => {
            total += record[k][r].time;
            count += 1;
        });
    });
    toast(`平均時間は${returnTime(total / count)}です。`);
}
...
function exportBackup() {
    if(!data.recordList) {
        toast("データがないためバックアップを取れません。");
        return;
    }
    const jsonString = JSON.stringify(data.recordList);
    const blob = new Blob([jsonString], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `time_note_backup_${returnDate(Date.now()).replaceAll("/", "-")}.json`;
...
function importBackup() {
    const fileEle = document.getElementById("backup-file-input");
    const file = fileEle.files[0];
    const reader = new FileReader();
    reader.onload = (e) => {
        const newData = JSON.parse(e.target.result);
        if(typeof newData !== "object" || newData === null) {
            toast("選択されたファイルに有効なデータが含まれていません。");
            return;
        }
        data.recordList = newData;
        dataSave();
        dataApply();
        const showFooterEle = document.getElementById("show-footer");
        showFooterEle.checked = false;
    };
...
window.onload = function() {
    ...
    const versionLabels = document.querySelectorAll(".version-button");
    versionLabels.forEach((l) => {
    ...
    const averageButtonEle = document.getElementById("average-button");
    averageButtonEle.addEventListener("click", () => {
        average();
    });
    /*const sortButtonEle = document.getElementById("sort-button");
    sortButtonEle.addEventListener("click", () => {
        sortPopup();
    });*/
    const exportBackupButtonEle = document.getElementById("export-backup-button");
    exportBackupButtonEle.addEventListener("click", () => {
        toast("バックアップファイルを作成してダウンロードしますか？", exportBackup);
    });
    const importBackupButtonEle = document.getElementById("import-backup-button");
    importBackupButtonEle.addEventListener("click", () => {
        const fileEle = document.getElementById("backup-file-input");
        if (fileEle.files[0]) {
            toast("バックアップファイルを適用しますか？適用すると現在のデータはリセットされます。", importBackup);
        } else {
            toast("ファイルを選択してください。");
        }
    });
